...
df = CSV.read("data/phase_times.csv", DataFrame)
...
# --- Extract fractions for one dataset at one time ---
function phase_fractions(df::DataFrame, target_time::Real)
    df_t = df[df.time .== target_time, :]

    if nrow(df_t) == 0
        error("No data for time = $target_time")
    end
    ...
    if N == 0
        error("Nalive == 0 at time $target_time, cannot compute fractions.")
    end
...
# --- Make ONE plot per phase, comparing all particles ---
function compare_particles_by_phase(frac_dicts::Dict{String, Dict{Symbol, Float64}},
                                    phases::Vector{Symbol}=[:G1, :S, :G2, :M])

    plots = Vector{Plots.Plot}(undef, length(phases))

    for (i, phase) in enumerate(phases)
        particle_labels = collect(keys(frac_dicts))
        fractions       = [frac_dicts[p][phase] for p in particle_labels]

        plt = bar(particle_labels, fractions;
                    ylim    = (0, 1),
                    ylabel  = "Fraction",
                    title   = "Phase $phase",
                    legend  = false,
    ...
    return plot(plots...; layout=(2, 2), size=(1400, 900))
end
...
frac_data = Dict{String, Dict{Symbol, Float64}}(
    "12C_15"  => phase_fractions(df_12C_15,  target_time),
    "12C_20"  => phase_fractions(df_12C_20,  target_time),
    "12C_80"  => phase_fractions(df_12C_80,  target_time),
    "1H_2"    => phase_fractions(df_1H_2,    target_time),
    "1H_10"   => phase_fractions(df_1H_10,   target_time),
    "1H_100"  => phase_fractions(df_1H_100,  target_time)
)

p_phases = compare_particles_by_phase(frac_data)
