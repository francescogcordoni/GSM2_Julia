...
NTCP_all = Array{Float64}(undef, length(dose_array), length(NFraction), length(O2_array), length(s))
...
for i_frac in eachindex(NFraction)
    ...
    for i_D in eachindex(dose_vec)
        ...
        E        = 80.0         # Ion kinetic energy (MeV/u)
        particle = "12C"    
        ...
        for i_O2 in eachindex(O2_array)
            ...
            #& Loop over seriality parameters
            for i_s in eachindex(s)
                #& Compute NTCP and dose
                NTCP_all[i_D, i_frac, i_O2, i_s], D_voxel_all[i_D, i_frac] = compute_NTCP_Box_s(VoxArray, N_sideVox, s[i_s])
            end

            #& Compute TCP
            TCP_all[i_D, i_frac, i_O2] = compute_TCP_df(cell_df_temp)
...
#O2
plt_NTCP = Plots.plot(D_voxel_all[:, 1].*NFraction[1], NTCP_all[:, 1, 1, 1], xlabel = "Total dose [Gy]", ylabel = "NTCP", label = label_legend[1])
plt_NTCP = Plots.plot!(D_voxel_all[:, 1].*NFraction[1], NTCP_all[:, 1, 2, 1], xlabel = "Total dose [Gy]", ylabel = "NTCP", label = label_legend[2], xlims=(-0.99, 101))

#s
plt_NTCP = Plots.plot(D_voxel_all[:, 1].*NFraction[1], TCP_all[:, 1, 2], xlabel = "Total dose [Gy]", ylabel = "NTCP", label = "TCP")
for i_s in eachindex(s)
plt_NTCP = Plots.plot!(D_voxel_all[:, 1].*NFraction[1], NTCP_all[:, 1, 2, i_s], xlabel = "Total dose [Gy]", ylabel = "NTCP", label = s[i_s])
end
display(plt_NTCP)