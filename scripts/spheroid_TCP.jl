...
X_dam_array = Array{Array{Int64}}(undef, length(O2_array), length(NFraction))
Y_dam_array = Array{Array{Int64}}(undef, length(O2_array), length(NFraction))
SP_array = Array{Array{Float64}}(undef, length(dose_array), length(O2_array), length(NFraction))
...
for i_frac in eachindex(NFraction)
    ...
    for i_D in eachindex(dose_vec)
        ...
        X_dam_array[i_O2, i_frac] = map(sum, cell_df_copy.dam_X_dom[cell_df_copy.is_cell .> 0])
        Y_dam_array[i_O2, i_frac] = map(sum, cell_df_copy.dam_Y_dom[cell_df_copy.is_cell .> 0])
        ...
        Dcell_array[i_D, i_frac] = cell_df_copy.dose_cell[cell_df_copy.is_cell .> 0]

        #& Calculate survival probability after fraction
        Sp_NFr = Sp.^NFraction[i_frac]
        SP_array[i_D, i_O2, i_frac] = Sp_NFr

        #& Compute TCP and dose
        D_voxel_all[i_D, i_frac] = mean(cell_df_copy.dose_cell[cell_df_copy.is_cell .== 1])
...
Plots.plot(D_voxel_all[:, 2].*NFraction[2], SP_array[:, 2, 1], label = "SP", legend = :best, yscale = :log10)

#& Plot TCP vs dose
Plots.plot(D_voxel_all[:, 2].*NFraction[2], TCP_all[:, 2, 1], label = "NTCP")