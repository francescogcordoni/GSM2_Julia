...
Efficiently gets the atomic number (Z) for a given ion string (e.g., "12C").
Uses a pre-compiled dictionary for fast lookups.

# Returns
- The atomic number `Z` if the particle is found.
- `-1` if the particle is not in the map.
"""
function getZ(ion::AbstractString)
    particle = filter(isletter, ion)
    return get(Z_MAP, particle, -1) # Return -1 if not found
end

"""
    load_stopping_power() -> Dict{String, Matrix{Float64}}

Loads stopping power data from text files into a dictionary.
"""
function load_stopping_power()
    sp = Dict{String,Matrix{Float64}}()
    for (ion, element) in [("1H", "H"), ("4He", "He"), ("6Li", "Li"), ("8Be", "Be"),
        ("10B", "B"), ("12C", "C"), ("14N", "N"), ("16O", "O")]
        sp[ion] = readdlm("stoppingpower/$(element)_water.txt", Float64)
    end
    return sp
end
...
function GenerateHit_BOX(X_box::Float64)
...
function GenerateHit_Circle(x_::Float64, y_::Float64, R_beam::Float64)
...
#Generate uniform Hit in a circle center in 0 0 from with radius cell.r+Rk
function GenerateHit(cell::Cell, Rk::Float64)
...
function GetRadialLinearDose(r::Float64, ion::AT, type::String)
    ...
    if type == "KC"
        ...
        if r <= ion.Rc
            D_arc = LETk
            D_arc -= 2 * π * Kp * log(r_penumbra / r_core)
            D_arc /= (π * r_core^2)
            D_arc *= 1.05
        else
            D_arc = Kp / (r * r)
    ...
    elseif type == "LEM"
        ...
        D_arc = 0.
        if r <= ion.Rc
            D_arc = (1 / (pi * Rc^2)) * (LETk / (1 * (1 + 2 * log(ion.Rp / ion.Rc))))
            D_arc *= 1.05
        else
            D_arc = (1 / (pi * r * r)) * (LETk / (1 * (1 + 2 * log(ion.Rp / ion.Rc))))
        end
    else
        println("Error - unknown method")
    end
...
function arc_intersection_angle(r::Float64, b::Float64, r_nucleus::Float64)
    if r <= abs(b - r_nucleus)
        return b < r_nucleus ? 2π : 0.0
    elseif r < b + r_nucleus
        arg = (b^2 + r^2 - r_nucleus^2) / (2 * b * r)
        return 2 * acos(clamp(arg, -1.0, 1.0))
    else
        return 0.0
    end
end
...
##############
function integrate_weighted_radial_track(rMin::Float64, rMax::Float64, b::Float64, r_nucleus::Float64, step::Int64, type::String, irrad_cond::AT)
    # Preallocate variables
    integral = 0.0
    area = 0.0

    # Handle the case where rMin is zero or negative
    log_rMin = rMin > 0 ? log10(rMin) : -5.0

    log_rMax = log10(rMax)
    nSteps = max(3, step) # Ensure at least 3 steps
    ...
    for i in 1:nSteps
        ...
        r2 = 10^log_r2
        # Ensure r2 doesn't exceed rMax (last step)
        if i == nSteps
            r2 = rMax
        end
        ...
        f = (r2 - r1) * (f1 + f2) / 2.0
        integral += f
        area += (r2 - r1) * (arc_weight1 * r1 + arc_weight2 * r2) / 2.0
        ...
        #println(r2 ,"\n")
        if r2 == rMax #exit from the loop if r2 is equal to rMax
            break
        end
    end
...
function distribute_dose_domain(x0::Float64, y0::Float64, radius::Float64, track::Track, irrad_cond::AT, type::String)
    ...
    if b <= radius
        ...
        if b + track.Rk < radius
            r_intersection = track.Rk
        else
            r_intersection = radius - b
        end

        area1 = π * r_intersection^2
        area, integral, Gyr = integrate_weighted_radial_track(0., r_intersection, b, radius, 500, type, irrad_cond)
        dose = integral
        # dose = track.getRadialIntegral(0.0, r_intersection) * area1

        if rMax > r_intersection
            area, integral, Gyr = integrate_weighted_radial_track(r_intersection, rMax, b, radius, 500, type, irrad_cond)
        ...
        if rMax == track.Rk
            if track.Rk > radius - b
                ...
                theta2 = acos((b / (2 * radius) - (rMax * rMax) / (2 * b * radius) + (radius) / (2 * b)))
                area3 = π * radius^2 - (theta1 * track.Rk^2 + theta2 * radius^2 - track.Rk * b * sin(theta1))
            else
    ...
    elseif b <= radius + track.Rk
        ...
        area, integral, Gyr = integrate_weighted_radial_track(rMin, rMax, b, radius, step, type, irrad_cond)
        dose = integral
        area2 = area
        if rMax == track.Rk
            theta1 = acos((b / (2 * rMax) + rMax / (2 * b) - (radius^2) / (2 * b * rMax)))
...
function domain_GSM2(X::Vector{Int64}, Y::Vector{Int64}, gsm2::GSM2)  #gsm2::GSM2
    ...
    for j in 1:size(X)[1]
        p = 1
        for i in X[j]:-1:1
            p *= (i * gsm2.r) / ((gsm2.r + gsm2.a) * i + gsm2.b * i * (i - 1))
        end
        p_cell *= p
    end
...
"""
Generate positions for cells in a 3D squared lattice.

X_box: Float64 - length of the box side.
R_cell: Float64 - radius of the cells.

Returns the total number of cells and their positions (x, y, z) in a vector of tuples.
"""
...
function generate_cells_positions_squaredlattice_3D_new(X_box::Float64, R_cell::Float64)
    ...
    # Spacing costante tra centri
    spacing = 2 * R_cell

    # Inizializza array di posizioni
    nodes_positions = Vector{Tuple{Float64,Float64,Float64}}()

    # Ciclo 3D per generare i centri
...
"""
Compute the neighbors of each cell in a 3D lattice.

N, M, and L are the dimensions of the lattice.

Returns a vector of vectors, where each inner vector contains the indices of the neighboring cells of the corresponding cell in the 1D lattice.
"""
...
"""
    create_cells_3D_voxel_df!(N::Int64, nodes_positions::Vector{Tuple{Float64,Float64,Float64}}, R_cell::Float64, SP_::Float64, gsm2::GSM2, cell_df::DataFrame, domain::Int64, tumor_radius::Float64, full_cycle::Bool, geometry::String, type::String, X_box::Float64, X_voxel::Float64)

Creates cells directly into `cell_df` without using `arrayOfCell`.
"""
function create_cells_3D_voxel_df!(N::Int64, nodes_positions::Vector{Tuple{Float64,Float64,Float64}}, R_cell::Float64, SP_::Float64, gsm2::GSM2, cell_df::DataFrame, domain::Int64, tumor_radius::Float64, full_cycle::Bool, geometry::String, type::String, X_box::Float64, X_voxel::Float64)
    ...
    # Initialize vectors to populate DataFrame columns
    ...
    Threads.@threads for i in 1:N
        ...
        # Optimized voxel index calculation (assuming regular grid)
        # nodes_positions often generated with -X_box + R_cell + index * 2*R_cell
        # We can compute indices directly instead of searching ranges if the grid is regular.
        # However, to be safe and consistent with previous logic, let's just do a optimized calculation
        # X_box is half-width? Previous code had: x_range = -X_box + (i-1)*X_voxel to -X_box + i*X_voxel
        # So x relative to box start (-X_box) is x + X_box. 
        # Index is floor((x + X_box) / X_voxel) + 1
    ...
    cell_df.O = O_vec # Note: Original code initialized O=5 here, but later set_oxygenation overrides it.
    cell_df.i_voxel_x = i_voxel_x_vec
    cell_df.i_voxel_y = i_voxel_y_vec
    cell_df.i_voxel_z = i_voxel_z_vec

    # Initialize other columns that Cell struct had but were not in DF or initialized to empty/zero
    # DataFrame creation in Run_tidy.jl might not have these columns yet, or we assume they exist.
    # Typically Run_tidy creates cell_df with just x,y,z,index.
    # We need to ensure we don't error if columns don't exist, or we create them.
    # In Julia DataFrames, assigning to new property creates it.

    # Initialize damage and arrays. 
    # Note: Cell struct had Matrix{Float64} for dam_X/dam_Y. 
    # In DataFrame we can store them, but usually for performance we separate complex structs.
    # However, create_cells populated `dam_X` with empty matrix.
...
"""
    create_cells_3D_voxel_1layervoxel_df!(N::Int64, nodes_positions::Vector{Tuple{Float64,Float64,Float64}}, R_cell::Float64, SP_::Float64, gsm2::GSM2, cell_df::DataFrame, domain::Int64, tumor_radius::Float64, full_cycle::Bool, geometry::String, type::String, N_sideVox::Int64, N_CellsSide::Int64, X_box::Float64, X_voxel::Float64)

Creates cells directly into `cell_df` without using `arrayOfCell` (1-layer voxel version).
"""
...
"""
    populate_cells_wrapper(ParIrr::String, N::Int64, nodes_positions::Vector{Tuple{Float64,Float64,Float64}}, R_cell::Float64, SP_::Float64, gsm2::GSM2, cell_df::DataFrame, domain::Int64, tumor_radius::Float64, full_cycle::Bool, target_geom::String, type::String, N_sideVox::Int64, N_CellsSide::Int64, X_box::Float64, X_voxel::Float64)

Wrapper function to populate `cell_df` based on irradiation type.
"""
function populate_cells_wrapper(ParIrr::String, N::Int64, nodes_positions::Vector{Tuple{Float64,Float64,Float64}}, R_cell::Float64, SP_::Float64, gsm2::GSM2, cell_df::DataFrame, domain::Int64, tumor_radius::Float64, full_cycle::Bool, target_geom::String, type::String, N_sideVox::Int64, N_CellsSide::Int64, X_box::Float64, X_voxel::Float64)
...
function compute_energy_box!(irrad_cond::Array{AT}, ion::Ion, irrad::Irrad, type_AT::String, cell_df::DataFrame, track_seg::Bool)
    ...
    for i in 1:size(unique_z, 1)
        irrad_cond[i] = AT(particle, E, A, Z, LET, 1., Rc, Rp, Rk, Kp)
        if (!track_seg) & (E != 0.)
            (E, LET) = residual_energy_after_distance(E, Z, A, diff(unique_z)[1], particle, sp)
        end
        ion = Ion(particle, E, A, Z, LET, 1.0)
        Rc, Rp = ATRadius(ion, irrad, type_AT)
        Rk = Rp
    end
    ion = ion_original
end
...
function compute_cell_survival_GSM2!(cell_df::DataFrame, gsm2::GSM2, NFrac::Int64)
...
"""
    MC_loop_copy_dose_domain_layer_fast!(cell_df::DataFrame, at_single::DataFrame, at::DataFrame, energy_step_to_match::Int64)
    (Note: when track_seg=false, 'id' is actually 'energy_step_to_match')

Copies the calculated domain doses from representative cells in `at_single`.

- If `track_seg=true` (implicitly, as this function is called by `MC_loop_copy_dose_domain_fast!`),
  `at_single` contains doses for representative cells of a single (x,y) plane, and `energy_step_to_match` is not used for filtering by layer/energy_step here.
  The copying is based on matching (x,y) coordinates across all layers.
- If `track_seg=false`, `at_single` contains doses for representative cells of a specific `energy_step` (passed as `energy_step_to_match`),
  and the function copies these doses to all active cells in `cell_df` and `at` that belong to that `energy_step`.

If `:dose` or `:dose_cell` columns do not exist in `cell_df`, they will be created.

# Arguments
- `cell_df::DataFrame`: DataFrame containing information for ALL cells, including `index`, `x`, `y`, `layer`, `energy_step`, `is_cell`. Will be modified in-place.
- `at_single::DataFrame`: DataFrame containing calculated domain doses for REPRESENTATIVE cells of the specified `energy_step_to_match`. Must have an `index` column linking to `cell_df`. Domain columns (e.g., `center_1`, ...) hold the doses.
- `at::DataFrame`: Target DataFrame, structured like `at_single` but for ALL cells. This DataFrame will be modified in-place for rows matching `energy_step_to_match`.
- `energy_step_to_match::Int64`: The energy_step ID for which doses are being copied.
"""
function MC_loop_copy_dose_domain_layer_fast_notsc!(cell_df::DataFrame, at_single::DataFrame, at::DataFrame, energy_step_to_match::Int64)

    println("Copying doses for energy_step $energy_step_to_match...")
    ...
    if !hasproperty(cell_df, :x) || !hasproperty(cell_df, :y) || !hasproperty(cell_df, :layer) || !hasproperty(cell_df, :is_cell) || !hasproperty(cell_df, :energy_step)
        error("cell_df is missing required columns (:x, :y, :layer, :is_cell, :energy_step).")
    end
    ...
    if num_domains == 0 && nrow(at_single) > 0
        @warn "No domain columns found in 'at_single' (excluding :index) for energy_step $energy_step_to_match."
        # Proceed, but dose vectors will be empty
    elseif nrow(at_single) == 0
        @warn "'at_single' is empty for energy_step $energy_step_to_match. No doses to copy. Skipping copy for this energy_step."
    ...
    if nrow(at_single) == 0
        println("Finished copying doses for energy_step $energy_step_to_match (at_single was empty).")
    ...
    for row_single in eachrow(at_single)
        ...
        if cell_df_row_idx == 0
            @warn "Representative index $representative_index from at_single (energy_step $energy_step_to_match) not found in cell_df. Skipping."
            continue
        end

        # Ensure this representative cell is indeed in the current energy_step
        if cell_df.energy_step[cell_df_row_idx] != energy_step_to_match
            @warn "Representative index $representative_index from at_single (energy_step $energy_step_to_match) points to cell in energy_step $(cell_df.energy_step[cell_df_row_idx]) in cell_df. Skipping."
            continue
        end
    ...
    # --- Iterate through all rows of the main cell_df and copy doses for the target energy_step ---
    Threads.@threads for i in 1:num_cells
        # Check if the cell is in the target energy_step AND is active
        if cell_df.energy_step[i] == energy_step_to_match && cell_df.is_cell[i] == 1
            ...
            if dose_data === nothing
                @warn "Coordinates $coords (cell index $(cell_df.index[i]), energy_step $energy_step_to_match) not found in xy_to_dose_data map. Assigning zero dose."
            ...
            if at_row_idx == 0
                #@warn "Cell index $cell_idx (energy_step $energy_step_to_match) not found in main 'at' DataFrame. Cannot update 'at'."
            else
                if length(names(at, Not(:index))) != num_domains
                    @error "Number of domain columns in main 'at' DataFrame ($(length(names(at, Not(:index))))) does not match num_domains from at_single ($num_domains) for energy_step $energy_step_to_match. Cannot update 'at'."
    ...
    println("Finished copying doses for energy_step $energy_step_to_match.")
end
...
"""
function MC_loop_copy_dose_domain_fast!(cell_df_::DataFrame, at_single::DataFrame, at::DataFrame)

    println("Copying doses from representative plane to all layers...")

    # --- Input Validation & Column Initialization ---
    if !hasproperty(cell_df_, :index) || !hasproperty(at_single, :index) || !hasproperty(at, :index)
        error("One or more DataFrames are missing the required 'index' column.")
    end
    if !hasproperty(cell_df_, :x) || !hasproperty(cell_df_, :y)
        error("cell_df_ is missing the required 'x' or 'y' columns.")
    end
    ...
    num_domains = length(domain_cols)
    if num_domains == 0 && nrow(at_single) > 0
        @warn "No domain columns found in 'at_single' (excluding :index)."
        # Decide how to proceed - error, or initialize with empty/zero vectors?
        # Let's initialize with empty for now, but this might indicate an issue.
        num_domains = 0
    elseif nrow(at_single) == 0
        @warn "'at_single' is empty. No doses to copy. 'cell_df_.dose' and 'cell_df_.dose_cell' might remain uninitialized or empty."
        # Initialize columns if they don't exist, but they will be empty/zero
        num_domains = 0 # Or a default if known
    end


    # Check and initialize :dose column
    if !hasproperty(cell_df_, :dose)
        println("Initializing cell_df_.dose column")
        # Initialize with vectors of the correct size (or empty if num_domains is 0)
        cell_df_.dose = [zeros(Float64, num_domains) for _ in 1:num_cells]
    elseif !(eltype(cell_df_.dose) <: AbstractVector) # Check if it holds vectors
        @warn "cell_df_.dose column exists but does not hold Vectors (type: $(eltype(cell_df_.dose))). Reinitializing."
        cell_df_.dose = [zeros(Float64, num_domains) for _ in 1:num_cells]
        # Optional: Check if existing vectors have the right size
        # elseif num_cells > 0 && length(cell_df_.dose[1]) != num_domains
        #     @warn "cell_df_.dose vectors have incorrect size. Reinitializing."
        #     cell_df_.dose = [zeros(Float64, num_domains) for _ in 1:num_cells]
    end

    # Check and initialize :dose_cell column
    if !hasproperty(cell_df_, :dose_cell)
        println("Initializing cell_df_.dose_cell column")
        cell_df_.dose_cell = zeros(Float64, num_cells)
    elseif !(eltype(cell_df_.dose_cell) <: AbstractFloat) # Check if it holds Floats
        @warn "cell_df_.dose_cell column exists but does not hold Floats (type: $(eltype(cell_df_.dose_cell))). Reinitializing."
        cell_df_.dose_cell = zeros(Float64, num_cells)
    end
    # --- End Initialization ---


    # --- Optimization: Pre-group cell_df_ indices by (x, y) coordinates ---
    xy_to_indices = Dict{Tuple{Float64,Float64},Vector{Int}}()
    sizehint!(xy_to_indices, nrow(at_single)) # Optional: give a hint for dictionary size

    # Use cell_df_ directly for grouping, assuming it contains all cells
    for row in eachrow(cell_df_)
        # Ensure coordinates are Float64 for consistent dictionary keys
        coords = (Float64(row.x), Float64(row.y))
        idx = row.index
        if haskey(xy_to_indices, coords)
            push!(xy_to_indices[coords], idx)
        else
            xy_to_indices[coords] = [idx]
        end
    end
    # --- End Optimization ---

    # Iterate through each row of the representative dose data (at_single)
    for row_single in eachrow(at_single)
        representative_index = row_single.index

        # Extract the vector of domain doses for this representative cell
        # Ensure it's a new vector copy if needed later, but direct use is fine here
        domain_doses_vector = Vector(row_single[domain_cols])

        # Calculate the scalar average dose for the representative cell
        # Use mean() from Statistics. Define how to handle empty vectors if that's possible.
        scalar_dose_cell = isempty(domain_doses_vector) ? 0.0 : mean(domain_doses_vector)

        # Find the (x, y) coordinates of this representative cell in cell_df_
        rep_row_idx_in_celldf_lookup = findfirst(==(representative_index), cell_df_.index)

        if rep_row_idx_in_celldf_lookup === nothing
            @warn "Representative index $representative_index from at_single not found in cell_df_. Skipping."
            continue
        end
        rep_x = Float64(cell_df_.x[rep_row_idx_in_celldf_lookup])
        rep_y = Float64(cell_df_.y[rep_row_idx_in_celldf_lookup])
        coords = (rep_x, rep_y)

        # Get all target indices sharing these coordinates using the precomputed map
        if !haskey(xy_to_indices, coords)
            # This might happen if the representative cell in at_single isn't actually in the main cell_df_ grouping
            @warn "Coordinates ($rep_x, $rep_y) derived from representative index $representative_index not found in xy_to_indices map. Skipping."
            continue
        end
        target_indices = xy_to_indices[coords]

        # Copy the doses to all target cells in the 'at' and 'cell_df_' DataFrames
        for target_idx in target_indices
            # Find the row index in the target 'at' DataFrame
            target_row_idx_in_at = findfirst(==(target_idx), at.index)
            # Find the row index in the main 'cell_df_' DataFrame
            target_row_idx_in_celldf = findfirst(==(target_idx), cell_df_.index)

            # Check if index was found in both DataFrames
            if target_row_idx_in_at === nothing || target_row_idx_in_celldf === nothing
                if target_row_idx_in_celldf === nothing
                    @warn "Target index $target_idx (from stack at $coords) not found in cell_df_.index. Skipping copy for this index."
                else # target_row_idx_in_at must be nothing
                    @warn "Target index $target_idx (from stack at $coords) not found in at.index. Skipping copy for this index."
                end
                continue
            end

            # 1. Assign the vector of doses to the corresponding row slice in 'at'
            # Using .= ensures broadcasting if the left side is a view/slice
            # Check if domain_cols is empty before assignment
            if !isempty(domain_cols)
                at[target_row_idx_in_at, domain_cols] .= domain_doses_vector
            end


            # Check if the target cell in cell_df_ is active before assigning dose to cell_df_
            if cell_df_.is_cell[target_row_idx_in_celldf] == 1
                # 2. Assign the vector of doses to the 'cell_df_.dose' column
                # Make a copy to avoid all rows pointing to the same vector object
                cell_df_.dose[target_row_idx_in_celldf] = copy(domain_doses_vector)

                # 3. Assign the calculated scalar average dose to 'cell_df_.dose_cell'
                cell_df_.dose_cell[target_row_idx_in_celldf] = scalar_dose_cell
            else
                # For inactive cells, ensure their dose and dose_cell in cell_df_ are zero
                if num_domains > 0
                    if length(cell_df_.dose[target_row_idx_in_celldf]) != num_domains
                        cell_df_.dose[target_row_idx_in_celldf] = zeros(Float64, num_domains)
                    else
                        cell_df_.dose[target_row_idx_in_celldf] .= 0.0
                    end
                else # num_domains is 0
                    cell_df_.dose[target_row_idx_in_celldf] = Float64[]
                end
                cell_df_.dose_cell[target_row_idx_in_celldf] = 0.0
            end
        end
    end

    println("Finished copying doses.")
    # No return value needed as 'at' and 'cell_df_' are modified in-place
end
...
Compute the positions of the centers of the spheres in a 2D lattice
with a given nucleus radius.
...
function set_energy_steps!(cell_df::DataFrame, irrad_cond::Vector{AT})
    ...
    bin_energy = 0. .* Array{Float64}(undef, size(cell_df, 1))
...
function set_energy_bins(value::Float64, bins::Vector{Float64})
    for i in 1:(length(bins)-1)
        ...
        if (i == 1 && value == lower) || (value > lower && value ≤ upper)
            return i
        end
    end
end

function remap_bins(bin_indices::Vector{Float64})
    mapping = Dict{Int,Int}()
    result = Int[]
    counter = 1

    for bin in bin_indices
        if !haskey(mapping, bin)
            mapping[bin] = counter
            counter += 1
        end
        push!(result, mapping[bin])
    end
...
function calculate_beam_properties(calc_type::String, target_geom::String, X_box::Float64, X_voxel::Float64, tumor_radius::Float64)
    if calc_type == "fast"
        R_beam = (X_voxel / 2) * sqrt(2) #Radius of the beam [μm]
        x_beam, y_beam = -(X_box - X_voxel / 2), -(X_box - X_voxel / 2) #Coordiante of the center of the beam
    elseif calc_type == "full"
        if target_geom == "circle"
            R_beam = tumor_radius * sqrt(2)
        elseif target_geom == "square"
            R_beam = X_box * sqrt(2) #Radius of the beam [μm]
        end
...
"""
    generate_cells_positions_selector(target_geom::String, ParIrr::String, X_box::Float64, R_cell::Float64, N_sideVox::Int64, N_CellsSide_default::Int64)

Wrapper function to select the appropriate cell position generation method based on geometry and irradiation parameters.
Returns (N, nodes_positions, N_CellsSide).
"""
