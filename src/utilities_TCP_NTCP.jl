...
# Compute NTCP for full irradiation - BOX MODEL xyz - fit version
function compute_NTCP_Box_s_fit(VoxArray::Vector{Array{Voxel,3}}, N_sideVox::Int64, sj::Float64)
    Prod = 1.
    sumDose = 0.

    for i in 1:N_sideVox
                #FSU response
                P_FSU = VoxArray[i][1].ni;
                
                #Prod *= (1.0 - (P_FSU)^(sj)) #original Kallman
                Prod *= ((1.0 - P_FSU)^(sj)) #OK

                #Dose
                sumDose += VoxArray[i][1].Dose
    end

    NTCP_ = (1 - Prod)^(1/sj)
    DOSE = sumDose/N_sideVox
